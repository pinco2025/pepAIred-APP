...
class _TestInterfaceScreenState extends State<TestInterfaceScreen> {
  ...
  Future<void> _initializeTest() async {
    try {
      ...
      final adaptedTest = LocalTest(
        ...
        title: data.title,
        description:
            "${(data.duration / 60).floor()} minutes | ${data.totalMarks ?? 0} Marks",
      ...
      setState(() {
        ...
        _questionStatuses = List.filled(
          adaptedTest.questions.length,
  ...
  Future<void> _initializeSession(LocalTest testData) async {
    ...
    final existingTest = await SupabaseService.getExistingTestSession(
      user.id,
      testData.testId,
    );
    ...
    if (existingTest != null) {
      ...
      final newTest = await SupabaseService.createTestSession(
        user.id,
        testData.testId,
      );
      if (newTest != null) {
  ...
  void _handleMarkForReview() {
    setState(() {
      if (_questionStatuses[_currentQuestionIndex] !=
        ...
        final currentQ = _testData!.questions[_currentQuestionIndex];
        _questionStatuses[_currentQuestionIndex] =
            _answers.containsKey(currentQ.uuid)
  ...
  Future<void> _saveProgress() async {
    ...
    await prefs.setString(
      'test-answers-${_testData!.testId}',
      json.encode(_answers),
    );
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      if (_studentTestId != null) {
        final success = await SupabaseService.submitTest(
          _studentTestId!,
          _answers,
        );
        if (success) {
            ...
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(
                builder: (_) => Scaffold(
                  ...
                  body: const Center(
                    child: Text("Your test has been submitted successfully!"),
                  ),
                ),
              ),
            );
  ...
  Map<String, int> get _sectionIndices {
    ...
    for (var section in _testData!.sections) {
      ...
      count += _testData!.questions
          .where((q) => q.section == section.name)
          .length;
    }
    return indices;
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_testData == null) {
      return const Scaffold(
        body: Center(child: Text("Error loading test data")),
      );
    }
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Wrap(
                      ...
                      runSpacing: 8,
                      alignment: WrapAlignment.center,
                      children: List.generate(_testData!.questions.length, (
                        index,
                      ) {
                        ...
                        } else if (_questionStatuses[index] ==
                            QuestionStatus.notAnswered)
                        ...
                        return InkWell(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: color,
                              border: _currentQuestionIndex == index
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  onPressed:
                      _currentQuestionIndex < _testData!.questions.length - 1
                      ? _handleNext
                      : null,
